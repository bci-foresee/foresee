...
class Pipeline:
    ...
    def cross_correlation(self, inputs, num_channels, num_samples):
        ...
        results_ptr = self.kernels.xcorr(inputs_ctypes, num_channels,
                                         num_samples)

        num_correlations = (num_channels * (num_channels - 1)) >> 1
        results = np.ctypeslib.as_array(results_ptr,
                                        shape=(num_correlations, ))
    ...
    def fft(self, signal_in, num_points=8000):
        ...
        signal_in_ctypes = signal_in.ctypes.data_as(
        ...
        if num_points == 1024:
            ...
            # convert the result pointer to a numpy array
            result_spectrum = np.ctypeslib.as_array(result_ptr,
                                                    shape=(16000, ))
    ...
    def bbf(self, signal_in, filter_vals, gain, num_points=8000):
        ...
        # input types
        self.kernels.butterworth_filter.argtypes = [
    ...
    def svm_predict(self, weights, inputs, size):
        ...
        weights_ctypes = weights.ctypes.data_as(ctypes.POINTER(
            ctypes.c_double))
        inputs_ctypes = inputs.ctypes.data_as(ctypes.POINTER(ctypes.c_uint16))

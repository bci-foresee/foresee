...
class patient_data_info:
    ...
    def __init__(self, sample_rate, seizure_begin, seizure_end, offset_beg,
                 offset_end):
        self.sample_rate = sample_rate
        self.seizure_begin = seizure_begin
        self.seizure_end = seizure_end
        self.gen_indices(offset_beg=offset_beg, offset_end=offset_end)
    ...
    def gen_indices(self, file_idx_len=1843200, offset_beg=0, offset_end=0):
        ...
        for i in range(len(self.seizure_begin)):
            ...
            overall_start_idx = np.floor(
                self.seizure_begin[i] * self.sample_rate[0][0]) + offset_beg
        ...
        for i in range(len(self.seizure_begin)):
            overall_end_idx = np.ceil(
                self.seizure_end[i] * self.sample_rate[0][0]) + offset_end
            #print(self.seizure_begin[i], self.sample_rate[0][0], self.seizure_begin[i] * self.sample_rate[0][0])

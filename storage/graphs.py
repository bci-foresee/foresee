...
def channel_freq_plot(model: StorageModel, result_type: ResultType):
    ...
    for i, freq in enumerate(frequencies):
        for j, channel in enumerate(channels):
            ...
            result = (model.get_result(ResultType.TOTAL_ENERGY) / (8192 / (freq * channel))) + model.get_result(ResultType.LEAKAGE_POWER)
    ...
    scatter1 = ax.scatter(frequencies_flat[colors_flat == 'green'], channels_flat[colors_flat == 'green'], results_flat[colors_flat == 'green'], c='green', label='Satisfy Latency Limit')
    scatter2 = ax.scatter(frequencies_flat[colors_flat == 'red'], channels_flat[colors_flat == 'red'], results_flat[colors_flat == 'red'], c='red', label='Do Not Satisfy Latency Limit')
...
def plot_power_vs_memory_types(model: StorageModel):
    ...
    for cell_type in memory_types:
        ...
        results['Dynamic Write Power'].append(model.get_result(ResultType.TOTAL_DYNAMIC_WRITE_POWER))
        results['Dynamic Read Power'].append(model.get_result(ResultType.TOTAL_DYNAMIC_READ_POWER))
        results['Leakage Power'].append(model.get_result(ResultType.LEAKAGE_POWER))
    ...
    for i, key in enumerate(results):
        ax.bar(x + (i - 1.5)*width, results[key], width, label=key, color=colors[i], hatch=hatches[i], edgecolor='black', zorder=3)

    ax.set_xlabel('Cell Type')
    ax.set_ylabel('Power (mW)')
    
    ax.set_xticks(x)
    ax.set_xticklabels([cell_type.value for cell_type in CellType], rotation=45)
    ...
    plt.subplots_adjust(left=0.28, bottom=0.146, right=0.788, top=0.874, wspace=0.2, hspace=0.2)
...
def plot_latency_vs_memory_types(model: StorageModel):
    ...
    for cell_type in memory_types:
        ...
        results['Total Latency'].append(model.get_result(ResultType.TOTAL_LATENCY))
        results['Write Latency'].append(model.get_result(ResultType.TOTAL_WRITE_LATENCY))
        results['Read Latency'].append(model.get_result(ResultType.TOTAL_READ_LATENCY))
...
def plot_life_expectancy_vs_memory_types(model: StorageModel):
    ...
    minor_locator = LogLocator(base=10.0, subs=np.arange(2, 10) * 0.1, numticks=20)
    ax.yaxis.set_minor_locator(minor_locator)
    ax.grid(which='major', axis='y', linestyle='-', color='black', linewidth=0.7, zorder=0)
    ax.grid(which='minor', axis='y', linestyle=':', color='gray', linewidth=0.5, zorder=0)

    for i, (min_val, max_val) in enumerate(zip(mins, maxes)):
        ax.bar(categories[i], max_val - min_val, bottom=min_val, color='limegreen', width=0.5, edgecolor='black', alpha=1.0, zorder=3)
...
def channel_freq_plot_vs_word_size(model: StorageModel, result_type: ResultType):
    frequencies = np.arange(10000, 100001, 10000)
    ...
    for i, freq in enumerate(frequencies):
        for j, channel in enumerate(channels):
            ...
            model.run()
            result_data_16[j, i] = (model.get_result(ResultType.TOTAL_ENERGY) / (8192 / (freq * channel))) + model.get_result(ResultType.LEAKAGE_POWER)

            model.update_config('word_width', 32)
            model.run()
            result_data_32[j, i] = (model.get_result(ResultType.TOTAL_ENERGY) / (8192 / (freq * channel))) + model.get_result(ResultType.LEAKAGE_POWER)

            model.update_config('word_width', 64)
            model.run()
            result_data_64[j, i] = (model.get_result(ResultType.TOTAL_ENERGY) / (8192 / (freq * channel))) + model.get_result(ResultType.LEAKAGE_POWER)
    ...
    ax.plot_surface(F, C, result_data_16, color=colors[0], alpha=0.7, label='word_width=16')
    ax.plot_surface(F, C, result_data_32, color=colors[1], alpha=0.7, label='word_width=32')
    ax.plot_surface(F, C, result_data_64, color=colors[2], alpha=0.7, label='word_width=64')
...
def plot_pipeline_stages(model: StorageModel, result_type: ResultType):
    ...
    ax.grid(True, which='major', linestyle='-', linewidth=0.7, alpha=0.8, zorder=0)
    ax.grid(True, which='minor', linestyle='--', linewidth=0.5, alpha=0.5, zorder=0)

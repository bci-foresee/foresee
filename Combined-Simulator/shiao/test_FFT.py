...
def setInputs(dut, input_real, input_imag):
    ...
    dut.fft_X0.value = input_real[0]
    dut.fft_X1.value = input_imag[0]
    dut.fft_X2.value = input_real[1]
    dut.fft_X3.value = input_imag[1]
    dut.fft_X4.value = input_real[2]
    dut.fft_X5.value = input_imag[2]
    dut.fft_X6.value = input_real[3]
    dut.fft_X7.value = input_imag[3]

    # print(dut.X0.value)

def debug_print(cycles, dut):
    print(f"cycles run: {cycles}, reset: {dut.fft_reset.value}, dut.next_out.value: {dut.fft_next_out.value}, Y0: {dut.fft_Y0.value}")
...
async def fft_test(dut):
    ...
    while not is_data_ready:
        await FallingEdge(dut.fft_clk) # this was it!!!! <----------------- when to latch data
